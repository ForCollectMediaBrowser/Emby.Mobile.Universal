...
namespace Emby.Universal.Core.Logging
{
    ...
    public class WinLogger : ILog
    {
        ...
        private static async Task<string> GetLogFileContent()
        {
            try
            {
                if (await _cacheStorage.FileExistsAsync(LogFileName))
                {
                    return await _cacheStorage.ReadAllTextAsync(LogFileName);
                }
            }
            catch (Exception)
            {
        ...
        public static async Task ClearLogs()
        {
            try
            {
                if (await _cacheStorage.FileExistsAsync(LogFileName))
                {
                    await _cacheStorage.DeleteFileAsync(LogFileName);
                }
            }
            catch (Exception)
            {
